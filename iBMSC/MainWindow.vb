...
Public Class MainWindow
    ...
    Public _rscratch As Boolean = False
    ...
    Private Sub ChangePlaySide(ByVal swap As Boolean)
        If _rscratch Then
            Columns.column(ColumnType.A1).BmsChannel = 1
            Columns.column(ColumnType.A2).BmsChannel = 2
            Columns.column(ColumnType.A3).BmsChannel = 3
            Columns.column(ColumnType.A4).BmsChannel = 4
            Columns.column(ColumnType.A5).BmsChannel = 5
            Columns.column(ColumnType.A6).BmsChannel = 8
            Columns.column(ColumnType.A7).BmsChannel = 9
            Columns.column(ColumnType.A8).BmsChannel = 6

            For i = 0 To UBound(Notes) Step 1
                If Notes(i).ColumnIndex = ColumnType.A1 Then
                    Notes(i).ColumnIndex = ColumnType.A8
                ElseIf Notes(i).ColumnIndex >= ColumnType.A2 AndAlso Notes(i).ColumnIndex <= ColumnType.A8 Then
                    Notes(i).ColumnIndex -= 1
                End If
            Next
        ElseIf swap Then
            Columns.column(ColumnType.A1).BmsChannel = 6
            Columns.column(ColumnType.A2).BmsChannel = 1
            Columns.column(ColumnType.A3).BmsChannel = 2
            Columns.column(ColumnType.A4).BmsChannel = 3
            Columns.column(ColumnType.A5).BmsChannel = 4
            Columns.column(ColumnType.A6).BmsChannel = 5
            Columns.column(ColumnType.A7).BmsChannel = 8
            Columns.column(ColumnType.A8).BmsChannel = 9
            For i = 0 To UBound(Notes) Step 1
                If Notes(i).ColumnIndex = ColumnType.A8 Then
                    Notes(i).ColumnIndex = ColumnType.A1
                ElseIf Notes(i).ColumnIndex >= ColumnType.A1 AndAlso Notes(i).ColumnIndex <= ColumnType.A7 Then
                    Notes(i).ColumnIndex += 1
                End If
            Next
        End If
    ...
    Private Sub ChangePlaySideSkin(ByVal swap As Boolean)
        Dim tLeft(8)
        For i = 0 To 7 Step 1
            tLeft(i) = Columns.column(ColumnType.A1 + i + 1).Left - Columns.column(ColumnType.A1 + i).Left
        Next
        If _rscratch Then
            Dim tcBG = Columns.column(ColumnType.A1).CBg
            Dim tcNote = Columns.column(ColumnType.A1).CNote
            Dim tcLNote = Columns.column(ColumnType.A1).CLNote
            Dim tcText = Columns.column(ColumnType.A1).CText
            Dim tcLText = Columns.column(ColumnType.A1).CLText
            Dim tWidth = Columns.column(ColumnType.A1).Width
            tLeft(8) = tLeft(0)
            For i = 0 To 6 Step 1
                Columns.column(ColumnType.A1 + i).CBg = Columns.column(ColumnType.A1 + i + 1).CBg
                Columns.column(ColumnType.A1 + i).CText = Columns.column(ColumnType.A1 + i + 1).CText
                Columns.column(ColumnType.A1 + i).CLText = Columns.column(ColumnType.A1 + i + 1).CLText
                Columns.column(ColumnType.A1 + i).SetNoteColor(Columns.column(ColumnType.A1 + i + 1).CNote)
                Columns.column(ColumnType.A1 + i).SetLNoteColor(Columns.column(ColumnType.A1 + i + 1).CLNote)
                Columns.column(ColumnType.A1 + i).Width = Columns.column(ColumnType.A1 + i + 1).Width
                tLeft(i) = tLeft(i + 1)
            Next
            Columns.column(ColumnType.A8).CBg = tcBG
            Columns.column(ColumnType.A8).CText = tcText
            Columns.column(ColumnType.A8).CLText = tcLText
            Columns.column(ColumnType.A8).SetNoteColor(tcNote)
            Columns.column(ColumnType.A8).SetLNoteColor(tcLNote)
            Columns.column(ColumnType.A8).Width = tWidth
            tLeft(7) = tLeft(8)
        ElseIf swap Then
            Dim tcBG = Columns.column(ColumnType.A8).CBg
            Dim tcNote = Columns.column(ColumnType.A8).CNote
            Dim tcLNote = Columns.column(ColumnType.A8).CLNote
            Dim tcText = Columns.column(ColumnType.A8).CText
            Dim tcLText = Columns.column(ColumnType.A8).CLText
            Dim tWidth = Columns.column(ColumnType.A8).Width
            tLeft(8) = tLeft(7)
            For i = 7 To 1 Step -1
                Columns.column(ColumnType.A1 + i).CBg = Columns.column(ColumnType.A1 + i - 1).CBg
                Columns.column(ColumnType.A1 + i).CText = Columns.column(ColumnType.A1 + i - 1).CText
                ...
                tLeft(i) = tLeft(i - 1)
            Next
            Columns.column(ColumnType.A1).CBg = tcBG
            Columns.column(ColumnType.A1).CText = tcText
            Columns.column(ColumnType.A1).CLText = tcLText
            Columns.column(ColumnType.A1).SetNoteColor(tcNote)
            Columns.column(ColumnType.A1).SetLNoteColor(tcLNote)
            Columns.column(ColumnType.A1).Width = tWidth
            tLeft(0) = tLeft(8)
        End If
        For i = 0 To 7 Step 1
            Columns.column(ColumnType.A1 + i + 1).Left = Columns.column(ColumnType.A1 + i).Left + tLeft(i)
        Next
    End Sub
    ...
    Private Sub UpdatePairing()
        ...
        Else
            ...
            For i = 1 To UBound(Notes)
                If Columns.IsColumnSound(Notes(i).ColumnIndex) AndAlso Notes(i).ColumnIndex < ColumnType.BGM Then
...
EndSearch:
    ...
    Private Sub TBChangePlaySide_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TBChangePlaySide.Click, mnChangePlaySide.Click
        _rscratch = sender.Checked
        ChangePlaySide(True)
        TBChangePlaySide.Checked = _rscratch
        mnChangePlaySide.Checked = _rscratch
...
StartCount:     If Not NtInput Then
    ...
    Public Sub CalculateTotalPlayableNotes()
        ...
        If Not NtInput Then
            For i = 1 To UBound(Notes)
                ...
                If Notes(i).ColumnIndex >= ColumnType.D1 And Notes(i).ColumnIndex <= ColumnType.D8 AndAlso Columns.column(Notes(i).ColumnIndex).IsVisible Then xIAll += 1
            Next
        ...
        Else
            For i = 1 To UBound(Notes)
                ...
                End If
                If (Notes(i).ColumnIndex >= ColumnType.D1 And Notes(i).ColumnIndex <= ColumnType.D8) AndAlso Columns.column(Notes(i).ColumnIndex).IsVisible Then
                    xIAll += 1
                    If Notes(i).Length <> 0 Then xIAll += 1
                End If
            Next
    ...
    Public Sub AppendNote(note As Note)
        ...
        If AutoincreaseWavIndex Then
            If Columns.IsColumnSound(note.ColumnIndex) Then
                IncreaseCurrentWav()
            ElseIf Columns.IsColumnImage(note.ColumnIndex) Then
                IncreaseCurrentBmp()
            End If
        End If
    ...
    Private Sub BWAVUp_Click(sender As Object, e As EventArgs) Handles BWAVUp.Click
        ...
        For i As Integer = xS To 1294
            ...
            If xIndex <> -1 Then
                ...
                For j = 1 To UBound(Notes)
                    If Columns.IsColumnSound(Notes(j).ColumnIndex) Then Continue For
    ...
    Private Sub BBMPUp_Click(sender As Object, e As EventArgs) Handles BBMPUp.Click
        ...
        For i As Integer = xS To 1294
            ...
            If xIndex <> -1 Then
                ...
                If Not _wavChangeLabel Then GoTo 1101

                Dim xL1 As String = C10to36(i)
                Dim xL2 As String = C10to36(i + 1)
                For j = 1 To UBound(Notes)
                    If Columns.IsColumnImage(Notes(j).ColumnIndex) Then Continue For
...
case2:              Dim xI0 As Integer
    ...
    Friend Sub SelectBmpFromNote(note As Note)
        LBMP.SelectedIndices.Clear()
        LBMP.SelectedIndex = C36to10(C10to36(note.Value \ 10000)) - 1
        ValidateBmpListView()
    End Sub
