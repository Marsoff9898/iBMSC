...
Partial Public Class MainWindow
    Private Sub OpenBms(xStrAll As String)
        ...
        For Each sLine In xStrLine
            ...
            ElseIf sLineTrim.StartsWith("#") And Mid(sLineTrim, 7, 1) = ":" Then 'If the line contains Ks
                ...
                If Columns.BMSChannelToColumn(channel, _rscratch) = 0 Then GoTo AddExpansion
        ...
        For Each sLine In xStrLine
            ...
            Dim channel As String = Mid(sLineTrim, 5, 2)
            If Columns.BMSChannelToColumn(channel, _rscratch) = 0 Then Continue For
            ...
            For i = 8 To Len(sLineTrim) - 1 Step 2 'For all Ks within that line ( - 1 can be ommitted )
                ...
                With Notes(UBound(Notes))
                    .ColumnIndex = Columns.BMSChannelToColumn(channel, _rscratch) +
    ...
    Private Function SaveBms() As String
        ...
        ' Add expansion text
        Dim expansionText = String.Format("{1}*---------------------- EXPANSION FIELD{1}{0}{1}", TExpansion.Text, vbCrLf)
        If TExpansion.Text = "" Then expansionText = ""

        ' Output main data field.
        Dim dataText = String.Format("{1}*---------------------- MAIN DATA FIELD{1}{0}{1}", measureString, vbCrLf)
    ...
    Private Function CreateBmsMeasureData(ByRef hasOverlapping As Boolean) As String
        ...
        For measureIndex = 0 To MeasureAtDisplacement(GreatestVPosition) + 1 'For i in each measure
            ...
            Dim notesInMeasure(upperLimit - lowerLimit + prevNotes.Count - 1) As Note

            ' Copy notes from previous array
            For i = 0 To prevNotes.Count - 1
            ...
            For i = 0 To relativeMeasurePos.Count - 1 'assign K texts
                ...
                If CInt(relativeMeasurePos(i) / xGcd) > UBound(xStrKey) Then
                    Dim note = New Note() With {
                        .ColumnIndex = Columns.BMSChannelToColumn(_bmsChannelList(currentBmsChannel), _rscratch),
    ...
    Private Function GenerateBackgroundTracks(measureIndex As Integer, ByRef hasOverlapping As Boolean,
        ...
        For ColIndex = ColumnType.BGM To greatestColumn 'Start rendering B notes (xI3 is columnindex)
            ...
            ret &= String.Format("#{0}01:{1}{2}", Add3Zeros(measureIndex), Join(notesString, ""), vbCrLf)
        Next
    ...
    Private Sub SaveiBmsc(path As String)
        ...
        Try
            ...
            If _rscratch Then xPref = xPref Or &H16
